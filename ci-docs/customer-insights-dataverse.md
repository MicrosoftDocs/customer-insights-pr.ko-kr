...
관리자는 Customer Insights 환경을 생성할 때 [기존 Dataverse 환경 사용](create-environment.md)으로 Customer Insights를 구성할 수 있습니다. Dataverse 환경에 대한 URL을 제공함으로써 새로운 Customer Insights 환경에 연결됩니다. Customer Insights 및 Dataverse 환경은 동일한 지역에서 호스팅되어야 합니다. 

기존 Dataverse 환경을 사용하지 않으려는 경우 시스템은 테넌트의 Customer Insights 데이터에 대한 새 환경을 생성합니다. 

> [!NOTE]
> 조직에서 이미 테넌트에서 Dataverse을 사용하는 경우 [Dataverse 환경 생성은 관리자가 제어](/power-platform/admin/control-environment-creation)를 기억하는 것이 중요합니다. 예를 들어 조직 계정으로 새 Customer Insights 환경을 설정하고 관리자가 관리자를 제외한 모든 사람에 대해 Dataverse 평가판 환경 생성을 비활성화한 경우 새 평가판 환경을 생성할 수 없습니다.
> 
