...
- **개인 소비자** 기본 대상 대상 그룹에 대해 환경을 구성해야 합니다.
...
1. 활동을 해당 의미 유형에 매핑한 후 **다음** 을 선택하여 계속 진행합니다.
...
1. 결과 페이지에는 5개의 기본 데이터 섹션이 있습니다.
    ...
    다음은 모델에 의해 생성된 제품 추천에 영향을 미칠 수 있는 요소입니다.
        - **과거 트랜잭션**: 과거 구매 패턴을 모델에서 활용하여 상품 추천을 생성했습니다. 예를 들어, 모델은 누군가가 최근에 *Surface Book 3* 와 *Surface Pen* 을 구매한 경우 *Surface Arc Mouse* 를 추천합니다. 이 모델은 기존에 많은 고객이 *Surface Book 3* 와 *Surface Pen* 을 구매한 후 *Surface Arc Mouse* 를 구매했다는 것을 학습했습니다.
