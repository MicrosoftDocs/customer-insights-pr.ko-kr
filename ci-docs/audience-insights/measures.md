...
측정값은 고객 행동과 비즈니스 성과를 더 잘 이해하는 데 도움이 됩니다. 측정값은 [통합 프로필](data-unification.md)에서 관련 가치를 확인합니다. 예를 들어, 비즈니스가 개별 고객의 구매 내역을 이해하기 위해 *고객당 총 지출* 을 보거나 전체 비즈니스의 총 수익을 이해하기 위해 *회사의 총 매출* 을 측정하고자 합니다.  
...
1. 데이터에 정수로 바꿔야 하는 값이 있는 경우(예: *Null* 을 *0* 으로 대체) **규칙** 을 선택합니다. 규칙을 구성하고 정수만 선택했는지 확인하십시오.
...
1. **기간 설정** 섹션에서 사용할 데이터의 시간 프레임을 정의합니다. 새 측정값이 전체 데이터 집합를 포함하도록 하려면 **항상** 을 선택하십시오. 또는 측정값이 **특정 기간** 에 초점을 맞추도록 할 수도 있습니다.
