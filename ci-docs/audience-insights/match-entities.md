...
1. Choose the custom match option from the **사용자 지정 유형** 드롭다운에서 사용자 지정 일치 옵션을 선택하고 **템플릿 다운로드** 를 선택합니다. 각 일치 옵션에 대해 별도의 템플릿이 필요합니다.

1. 다운로드한 템플릿 파일 항목입니다. 열고 세부 사항을 입력합니다. 템플릿에는 사용자 지정 일치에 사용할 엔터티 및 엔터티 기본 키 값을 지정하는 필드가 포함되어 있습니다. 예를 들어, *영업* 엔터티에서 기본 키 *12345* 가 *Contact* 엔터티에서 기본 키 *34567* 과 항상 일치하는 것을 원하는 경우 템플릿을 채웁니다.
    - Entity1: 영업
    - Entity1Key: 12345
    - Entity2: 연락처
    - Entity2Key: 34567
