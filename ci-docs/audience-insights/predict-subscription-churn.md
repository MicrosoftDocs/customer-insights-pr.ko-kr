...
1. **이탈을 예측할 미래의 일수** 에 숫자를 입력하여 이탈을 예측할 창을 설정합니다. 예를 들어 마케팅 유지 노력에 맞춰 향후 90일 동안 고객의 이탈 위험을 예측합니다. 더 길거나 짧은 기간 동안 이탈 위험을 예측하면 이탈 위험 프로필의 요인을 해결하기가 더 어려워질 수 있지만 이는 특정 비즈니스 요구 사항에 따라 크게 달라집니다. **다음** 을 선택하여 계속합니다.
   >[!TIP]
...
1. 오류 로그를 보려는 예측을 선택하고 **로그** 를 선택합니다.
   > [!div class="mx-imgBorder"]
   > ![닫기, 편집 모델 및 로그 버튼을 포함한 결과 메뉴 표시줄 보기](media/subscription-churn-logsbutton.PNG "닫기, 편집 모델 및 로그 버튼을 포함한 결과 메뉴 표시줄 보기")
1. 모든 오류를 검토합니다. 발생할 수 있는 여러 유형의 오류가 있으며 오류를 일으킨 조건을 설명합니다. 예를 들어, 정확하게 예측할 데이터가 충분하지 않다는 오류는 일반적으로 추가 데이터를 로드하여 해결됩니다.

## <a name="refresh-a-prediction"></a>예측 새로 고침

예측은 설정에서 구성된 대로 [데이터 새로 고침 예약](system.md#schedule-tab)에 자동으로 동일하게 갱신됩니다.
