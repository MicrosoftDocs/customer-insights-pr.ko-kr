...
ms.locfileid: "9304757"
---
# <a name="create-complex-segments-with-segment-builder"></a>세그먼트 빌더를 사용하여 복합 세그먼트 만들기

통합 고객 또는 통합 연락처 및 관련 엔터티에 대해 복잡한 필터를 정의합니다. 각 세그먼트는 처리 후 내보내고 조치를 취할 수 있는 고객 또는 연락처 레코드 집합을 만듭니다.

> [!TIP]
> **개별 고객** 을 기반으로 하는 세그먼트에는 세그먼트 구성원이 사용할 수 있는 연락처 정보가 자동으로 포함됩니다. **비즈니스 거래처** 에서 거래처와 연락처를 모두 [통합](data-unification.md) 하는 경우 세그먼트가 거래처 또는 비즈니스 연락처를 기반으로 할지 여부를 선택합니다. 연락처 정보를 예상하는 대상으로 내보내려면 연락처 세그먼트를 사용합니다. 거래처 정보를 예상하는 대상으로 내보내려면 거래처 세그먼트를 사용하십시오.
...
1. **새로 만들기** > **나만의 빌드** 를 선택합니다. 세그먼트 빌더 페이지에서 규칙을 정의하거나 구성합니다. 규칙은 고객 집합을 정의하는 하나 이상의 조건으로 구성됩니다.
   ...
   > [!NOTE]
   > 비즈니스 거래처를 기반으로 하는 환경의 경우 생성하려는 세그먼트 유형에 따라 **새로 만들기** > **거래처 세그먼트** 또는 **연락처 세그먼트(프리뷰)** 를 선택합니다. [거래처 계층 구조](relationships.md#set-up-account-hierarchies)가 정의되어 있고 하위 및 상위 관계를 기반으로 데이터를 필터링하는 규칙을 생성하려면 **계층 구조를 사용하시겠습니까?(프리뷰)** 를 선택하고 계층 구조를 선택한 다음 **적용** 을 선택합니다.
   >
   > :::image type="content" source="media/segment_acct_hierarchy.png" alt-text="세그먼트 선택 거래처 계층 구조 창":::
...
1. 규칙이 *고객* 엔터티(또는 B-to-B의 경우 *ContactProfile* 엔티티) 이외의 다른 엔터티를 사용하는 경우 **관계 경로 설정** 을 선택하여 선택한 엔터티를 통합 고객 엔터티에 매핑합니다. 가능한 관계 경로가 하나만 있는 경우 시스템이 자동으로 선택합니다. 다른 [관계 경로](relationships.md#relationship-paths)는 다른 결과를 산출할 수 있습니다. 모든 규칙은 고유한 관계 경로를 가질 수 있습니다.
...
1. 기본적으로 출력 엔터티에는 정의된 필터와 일치하는 고객 프로필의 모든 특성이 자동으로 포함됩니다. B-to-B에서 *ContactProfile* 엔터티를 사용할 때 거래처 ID가 자동으로 포함됩니다. 세그먼트가 *고객* 엔터티가 아닌 다른 엔터티를 기반으로 하거나 *ContactProfile* 의 더 많은 특성을 포함하려면 **프로젝트 특성** 을 선택하여 이러한 엔터티의 더 많은 특성을 출력 엔터티에 추가합니다.
   ...
   > [!NOTE]
   > - **프로젝트 특성** 은 *고객* 또는 *ContactProfile* 엔터티와 일대다 관계가 있는 엔터티에 대해서만 작동합니다. 예를 들어 한 고객이 여러 구독을 가질 수 있습니다.
   > - 프로젝션하려는 특성이 관계에 의해 정의된 대로 *고객* 또는 *ContactProfile* 엔터티에서 둘 이상의 홉 거리에 있는 경우 해당 특성은 작성 중인 세그먼트 쿼리의 모든 규칙에서 사용해야 합니다.
   > - 프로젝션하려는 특성이 *고객* 또는 *ContactProfile* 엔터티에서 한 홉만 떨어져 있으면 해당 특성이 작성 중인 세그먼트 쿼리의 모든 규칙에 있을 필요는 없습니다.
