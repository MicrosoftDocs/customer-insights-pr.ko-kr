...
3. **새 관계** 창에 다음 정보를 제공합니다.
   ...
   - **원본 카디널리티**: 원본 엔터티의 카디널리티입니다. 카디널리티는 집합에서 가능한 요소 수를 설명합니다. 항상 대상 카디널리티와 관련됩니다. **하나** 와 **다수** 중에서 선택할 수 있습니다. 다대일 및 일대일 관계만 지원됩니다.  
   ...
   - **목표 카디널리티**: 목표 엔터티 레코드의 카디널리티입니다.
   - **원본 키 필드**: 원본 엔터티의 외래 키 필드입니다. 예: SupportCase는 **CaseID** 를 외래 키 필드로 사용합니다.
   - **목표 키 필드**: 목표 엔터티의 키 필드입니다. 예: 고객은 **CustomerID** 를 키 필드로 사용합니다.

4. **저장** 을 선택하여 사용자 지정 관계를 만듭니다.

## <a name="set-up-account-hierarchies"></a>계정 계층 구조 설정

비즈니스 계정을 기본 목표 대상 그룹으로 사용하도록 구성된 환경에서 관련 비즈니스 계정에 대한 계정 계층을 구성할 수 있습니다. 예를 들어 별도의 사업부가 있는 회사입니다.
...
1. **계정 계층** 탭을 선택합니다.
1. **새 계정 계층** 을 선택합니다.
1. **계정 계층** 창에서 계층의 이름을 제공합니다. 시스템에서 출력 엔터티의 이름을 생성하지만 사용자는 이를 변경할 수 있습니다.
1. 계정 계층이 포함된 엔터티를 선택합니다. 일반적으로 계정이 포함된 동일한 엔터티에 있습니다.
1. 선택한 엔터티에서 **계정 UID** 및 **상위 계정 UID** 를 선택합니다.
1. **저장** 을 선택하여 계정 계층화를 마무리합니다.

## <a name="manage-existing-relationships"></a>기존 관계 관리

**관계** 페이지로 이동하여 생성된 모든 관계, 원본 엔터티, 목표 엔터티 및 카디널리티를 조회할 수 있습니다.

:::image type="content" source="media/relationships-list.png" alt-text="관계 페이지의 작업 표시줄에 있는 관계 및 옵션 목록입니다.":::

**필터링 기준** 또는 **관계 검색** 옵션을 사용하여 특정 관계를 찾을 수 있습니다. [**시각화 도우미**](#explore-the-relationship-visualizer)를 선택하여 기존 관계 및 해당 카디널리티의 네트워크 다이어그램을 확인합니다.

사용 가능한 작업을 보려면 관계를 선택합니다.
...
보기를 사용자 지정하려면 상자를 캔버스에서 끌어 상자의 위치를 변경하면 됩니다. 기타 옵션은 다음과 같습니다. 
...
관계 경로는 소스 엔터티와 대상 엔터티 간의 관계로 연결된 엔터티를 설명합니다. 통합 프로필 엔터티 이외의 다른 엔터티를 포함하는 세그먼트 또는 측정값을 생성할 때 사용되며 통합 프로필 엔터티에 도달하기 위한 여러 옵션이 있습니다. 다른 관계 경로는 다른 결과를 산출할 수 있습니다.
